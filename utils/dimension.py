# the general spacing
general_spacing = 20

# the general height of widgets
general_widget_height = 60

# the width of menu-list
menu_list_width = 250

# the size of main-window
main_window_size = (1400, 800)

# the size of meaning-form-dialog
meaning_form_dialog_size = (1200, 400)

# the size of example-form-dialog
example_form_dialog_size = (1200, 100)

# the size of conjugation-form-dialog
conjugation_form_dialog_size = (1600, 900)

# the size of base-message-box
base_message_box_size = (500, 100)

# the maximum height of example-list-widget
example_list_widget_max_height = 300

# the maximum height of example-translation-list-widget
example_translation_list_widget_max_height = 300
